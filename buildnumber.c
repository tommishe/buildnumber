...
int main(int argc, char* argv[])
{
  ...
  char header_file_path[255] = {0};
  char * header_file_name = "buildnumber.h";
  int i = 0;
  char line[80];
  size_t length = 0;
  ...
  if(argc > 1) 
  {
    ...
    else if(strncmp(argv[1], "--help", 6) == 0)
    {
	  ...
	  printf("  --file       specify where to find (and keep) the header file\n\n");
	...
	{
		unsigned int path_length = strlen( argv[2]);
		assert(path_length < 255);
		memcpy(&header_file_path[0],argv[2],path_length);
		strcat(&header_file_path[0], header_file_name);
	}
    ...
    if(argc > 2)
		printf ("Path to header file is: %s\n",header_file_path);    
  ...
  /* open the file for writing, creating it if necessary */
	...
	if(file == NULL) /* oops, can't create it, return an error status */
	{
		...
		fprintf(stderr, "Please check the path exists:%s",header_file_path); 
		return EXIT_FAILURE;
	}
